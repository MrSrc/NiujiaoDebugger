...
namespace DuiLib
{
	...
	void CContainerUI::ProcessScrollBar(RECT rc, int cxRequired, int cyRequired, bool bIsVirtualList)
	{
		if (m_pHorizontalScrollBar != NULL && m_pHorizontalScrollBar->IsVisible()) {
		...
		int aa = m_pVerticalScrollBar->GetScrollRange();
		...
		else
		{
			...
			if(InitOnce||!bIsVirtualList)  
			{
				InitOnce = false;
				RECT rcScrollBarPos = { rc.right, rc.top, rc.right + m_pVerticalScrollBar->GetFixedWidth(), rc.bottom };
				m_pVerticalScrollBar->SetPos(rcScrollBarPos);

				if (m_pVerticalScrollBar->GetScrollRange() != cyScroll&& !bIsVirtualList) {
