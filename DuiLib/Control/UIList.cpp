...
namespace DuiLib {
	...
	void CListUI::SetVirtualList(bool bIsVirtual)
	{
		m_pList->SetVirtualListAttr(bIsVirtual);
	}
	...
	void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("virtuallist")) == 0) SetVirtualList(_tcscmp(pstrValue, _T("true")) == 0);
	...
	CListBodyUI::CListBodyUI(CListUI* pOwner) : m_pOwner(pOwner), m_bIsVirtualList(false)
	{
		ASSERT(m_pOwner);
	}
	...
	void CListBodyUI::ProcessVLScrollbar(RECT rc, DWORD pos)
	{
		ProcessScrollBar(rc, 0, pos,m_bIsVirtualList);
	}

	void CListBodyUI::SetVirtualListAttr(bool bIsVirtualList)
	{
		m_bIsVirtualList = bIsVirtualList;
	}
	...
	void CListBodyUI::SetScrollPos(SIZE szPos)
	{
		...
		if(m_bIsVirtualList)  //niujiao
		{
			m_pManager->SendNotify(this, DUI_MSGTYPE_SCROLL, cx, cy);
		}
		else
		{
			RECT rcPos;
	...
	void CListBodyUI::SetPos(RECT rc)
	{
		...
		int iPosY = rc.top;
		if (m_pVerticalScrollBar && m_pVerticalScrollBar->IsVisible()&&!m_bIsVirtualList) {  //niujiao
		...
		ProcessScrollBar(rc, cxNeeded, cyNeeded,m_bIsVirtualList);
	}
